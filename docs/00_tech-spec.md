<!-- 版本 v0.4 | 日期 2026-06-07 | 狀態 draft | 對應 PRD v0.2（docs/PRD.md） | 專案 synergy(repo 根) | v0.4 變更：LINE OA 來訊自動分析（單次 Haiku 三合一）+ 語音經 OA push 發送 -->
...
| T08 | 語音草稿        | 文字草稿一鍵轉 TTS 語音（≤60 秒）；OA 客戶試聽後按發送直達 LINE，其他客戶下載自傳。 | OpenAI TTS 或 ElevenLabs（VoiceProvider 抽象） |
...
| PLT | 平台支撐        | 隱私同意、Freemium 配額、Care Streak、病毒擴散、學習紀錄；**LINE OA 收訊→新客戶 round-robin 輪派教練→來訊自動分析（單次 Haiku 三合一：情緒/事件/活檔案建議）→輔助回覆草稿（過合規）→教練審核→發送（文字/語音）**。 | 無 AI（純寫入）；輔助草稿與來訊分析走 Haiku 4.5 |
...
W3（追蹤 + 警報 + LINE 整合週）
  ...
  └─ LINE OA 整合（webhook 收訊 + round-robin 輪派 + 輔助回覆草稿 + 教練審核發送）
       ...
       ├─ 來訊自動分析（單次 Haiku 三合一：情緒/事件/活檔案建議，非同步）
       ├─ 輔助回覆草稿生成（Haiku 4.5，過合規 sidecar）
       └─ 教練審核後手動按「發送」→ push（reply token ~1 分鐘失效故走 push）

W4（語音 + 問卷 + Pilot 週）
  ├─ T08 語音草稿（VoiceProvider 抽象，TTS 供應商 W4 前選型；OA 客戶語音 push 發送）
...
- **LINE OA 來訊自動分析（inbound enrichment）**
  - 每則來訊寫入後，單次 Haiku 4.5 呼叫同時輸出三件事：情緒三檔（T03）、生活事件抽取（T02）、活檔案重點建議（T01）
  - 活檔案建議走「待確認」流程：教練確認才入檔，AI 不直接寫活檔案
  - OA 雙向往來自動構成活檔案互動時間軸（與手動補登並存、來源標示；自動紀錄原文不可改、僅能加註）
  - T04 太業務員警報：OA 發送訊息自動計入 streak，按發送第 3 則推銷型訊息前彈出預警
  - 分析成本計入該教練每日 AI 配額（G.5）；分析失敗不阻擋收訊主流程（Sentry 告警）
  - 加 OA 好友的歡迎訊息告知「訊息會由 AI 整理、不會自動回覆」（文字經法務確認）
- **LINE 語音訊息發送（T08 延伸）**
  - OA 客戶的語音草稿可由教練試聽後按「發送」→ LINE audio message push
  - 未試聽不可發；合規紅燈不可發；腳本修改須重新生成語音並重新過合規（不得舊語音配新腳本）
  - LINE 要求 m4a（AAC）格式；已發送語音 GCS 保存期延長為 30 天（未發送維持 7 天）
...
| **LINE push 發送**        | 教練按發送後 push 抵達          | < 5 秒               | 依 LINE Messaging API SLA；文字與語音相同 |
| **來訊自動分析**          | 來訊寫入後三合一分析完成        | < 5 秒（非同步）     | 不阻擋 webhook 200 回應；待回覆卡先出、分析結果補上 |
...
| **個資加密** | `raw_input`（貼上對話文字）與 `inbound_messages.raw_text`（OA 來訊原文）欄位 at-rest 加密；語音 storage_url 存 GCS（at-rest 採 GCS / DB 預設加密）                    |
| **HTTPS**    | 強制；開發環境除外                                                                                                                                                      |
...
| **LINE 相關新表**      | 新增 `official_accounts`（每租戶 OA 設定）、`inbound_messages`（收到的客戶訊息，含三合一分析結果欄位）、`contact_suggestions`（活檔案待確認建議：pending/confirmed/dismissed）；`contacts` 增 `line_user_id`、`source`、`assigned_at`；`message_drafts` 增 `line_oa`、`sent_at`、`delivery_method` 等欄位；`voice_clips` 增 `sent_at`、`line_message_id`、`retention_until`（已發送 30 天/未發送 7 天）；`today_tasks.source_type` 增 `inbound_reply` |
...
| **G.6** | LINE 發送人工審核硬限制 | LINE 訊息發送（含文字與語音）仍需教練按「發送」按鈕；AI 不自動回；每則訊息發送前過合規 sidecar；push 不繞過合規 Gate；語音須教練試聽後才可發送 |
| **G.7** | 來訊分析唯讀硬限制     | OA 來訊自動分析只產生建議（情緒/事件/活檔案待確認建議），永不觸發任何自動回覆或自動寫入活檔案；活檔案建議須教練確認才入檔             |
...
| 假設：LINE OA 免費額度      | 每租戶 LINE OA 免費 push 訊息額度有限（每月 200 則免費，超量計費）；語音與文字同計額度；Pilot 期 5 教練低流量預期不超標 | 若超量計費，需在成本控管加入 LINE push 費用計量                            |
| 假設：語音格式轉檔          | LINE audio message 要求 m4a（AAC）；假設 TTS 供應商可直接輸出 AAC，否則後端加 ffmpeg 轉檔步驟              | 影響 TTS 供應商選型（P0-06）與語音生成延遲目標                             |
| 假設：已發送語音保存 30 天  | 已透過 OA 發送的語音檔 GCS 保存 30 天（客戶可重播期），未發送維持 7 天；以 `retention_until` 欄位 + Lifecycle 雙軌控管 | 若 GCS 費用敏感可下修，但須同步修改對客戶的告知文字                        |
| 假設：來訊分析非同步        | 三合一分析為 webhook 200 之後的非同步任務，失敗不影響收訊與待回覆卡產生，僅 Sentry 告警                    | 若改同步會危及 LINE 3 秒回應要求，不建議                                   |
