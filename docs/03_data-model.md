...
版本 v0.3 | 日期 2026-06-07 | 狀態 draft | 對應 PRD v0.2（docs/PRD.md）/ 00_tech-spec v0.4 | 專案 synergy（repo 根）

> v0.3 變更：新增 `contact_suggestions` 表（3.30，活檔案待確認建議）；`inbound_messages` 增 enrichment 欄位；`voice_clips` 增 OA 發送欄位（listened_at / sent_at / line_message_id / retention_until）；`contact_interactions` 增 source 欄位（自動/手動來源標示）。
...
以下 Mermaid erDiagram 涵蓋全部 30 個實體及其關係（含 LINE 整合的 official_accounts、inbound_messages、contact_suggestions）。為可讀性，省略部分索引欄位，完整欄位定義見第 3 章。
...
CREATE TABLE care.contact_interactions (
    ...
    source           text NOT NULL DEFAULT 'manual' CHECK (source IN ('manual','line_oa')),
                     -- manual：教練手動補登（可編輯）；line_oa：OA 收發自動寫入（原文不可改，僅可加註）
    summary          text,
    raw_input        text,   -- PII，application-level 加密
    note             text,   -- 教練加註（line_oa 自動紀錄唯一可寫欄位）
...
| source | text | NOT NULL DEFAULT 'manual', CHECK | manual（手動補登，可編輯）/ line_oa（OA 自動寫入，原文不可改僅可加註）|
| summary | text | nullable | 互動摘要（AI 或手填） |
| raw_input | text | nullable | 原始輸入（PII，加密存） |
| note | text | nullable | 教練加註（line_oa 自動紀錄唯一可寫欄位）|
...
**用途**：T08 語音草稿，TTS 生成的語音檔記錄。`storage_url` 為 GCS 物件路徑（m4a/AAC，LINE audio message 要求）。保存期雙軌：未發送 7 天自動刪除；已透過 OA 發送保存至 `retention_until`（sent_at + 30 天，客戶重播期），由 APScheduler 清理 job 依 `retention_until` 執行刪除。
...
CREATE TABLE care.voice_clips (
    ...
    listened_at      timestamptz,           -- 教練試聽時間（OA 發送前置條件）
    sent_at          timestamptz,           -- 經 OA push 發送時間（nullable）
    line_message_id  text,                  -- LINE push 回傳 message id（nullable）
    retention_until  timestamptz,           -- 已發送：sent_at + 30d；未發送：NULL（依 expires_at）
    expires_at       timestamptz NOT NULL,  -- 建立後 7 天（未發送的刪除依據）
    created_at       timestamptz NOT NULL DEFAULT now()
);
```
...
| listened_at | timestamptz | nullable | 教練試聽時間；OA 發送的前置條件（未試聽回 422 VOICE_NOT_LISTENED）|
| sent_at | timestamptz | nullable | 經 OA push 發送時間 |
| line_message_id | text | nullable | LINE push 回傳 message id |
| retention_until | timestamptz | nullable | 已發送語音保存期限（sent_at + 30 天）；NULL 表示未發送、依 expires_at 刪除 |
| expires_at | timestamptz | NOT NULL | 未發送語音到期時間（建立後 7 天） |
| created_at | timestamptz | NOT NULL DEFAULT now() | — |

**索引**：
```sql
CREATE INDEX idx_voice_clips_distributor_id ON care.voice_clips(distributor_id);
-- APScheduler 清理到期語音（未發送：expires_at；已發送：retention_until）
CREATE INDEX idx_voice_clips_expires_at ON care.voice_clips(expires_at) WHERE sent_at IS NULL;
CREATE INDEX idx_voice_clips_retention ON care.voice_clips(retention_until) WHERE sent_at IS NOT NULL;
```
...
CREATE TABLE care.inbound_messages (
    ...
    enrichment_status    text NOT NULL DEFAULT 'pending'
                         ...
                         -- skipped：成本熔斷時跳過分析
    enriched_at          timestamptz,            -- 三合一分析完成時間
    detected_emotion     text CHECK (detected_emotion IN ('stressed','neutral','happy')),
    created_at           timestamptz NOT NULL DEFAULT now()
);
```

> `text` 欄位（來訊原文）為高敏感 PII，比照 `contact_interactions.raw_input` 做 application-level 加密。
...
| enrichment_status | text | NOT NULL DEFAULT 'pending', CHECK | pending / done / failed / skipped（熔斷跳過）|
| enriched_at | timestamptz | nullable | 三合一分析完成時間 |
| detected_emotion | text | nullable, CHECK | 分析出的情緒三檔（同步寫回 contacts.current_emotion）|
| created_at | timestamptz | NOT NULL DEFAULT now() | — |
...
LINE OA 收到訊息
  ...
  → INSERT inbound_messages（status='new', enrichment_status='pending'）
  → 通知教練（WebSocket/SSE push）
  → 非同步來訊分析（單次 Haiku 三合一：情緒/生活事件/活檔案建議）
      → UPDATE inbound_messages SET enrichment_status='done', enriched_at, detected_emotion
      → UPDATE contacts SET current_emotion
      → INSERT life_events / today_tasks（若抽到事件）
      → INSERT contact_suggestions（status='pending'，待教練確認）
      → 失敗：enrichment_status='failed'，Sentry 告警，不影響主流程
      → 熔斷：enrichment_status='skipped'
...
### 3.30 contact_suggestions（活檔案待確認建議）

**用途**：OA 來訊三合一分析抽出的活檔案重點建議。G.7 唯讀硬限制：AI 只寫本表（status='pending'），須教練 confirm 才 patch 進 `contacts` 對應欄位並重算 embedding；dismiss 則不入檔。confirm/dismiss 寫入 learning_logs 作為抽取準確率素材。

**ID 前綴**：`sug_`

```sql
CREATE TABLE care.contact_suggestions (
    id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id           uuid NOT NULL REFERENCES care.tenants(id),
    contact_id          uuid NOT NULL REFERENCES care.contacts(id) ON DELETE CASCADE,
    distributor_id      uuid NOT NULL REFERENCES care.distributors(id),
    inbound_message_id  uuid NOT NULL REFERENCES care.inbound_messages(id),
    field               text NOT NULL CHECK (field IN (
                            'health_concerns','family_context','interests','life_status')),
    suggested_value     text NOT NULL,
    evidence            text,    -- 來訊摘錄（佐證），PII 比照加密
    status              text NOT NULL DEFAULT 'pending'
                        CHECK (status IN ('pending','confirmed','dismissed')),
    confirmed_value     text,    -- 教練可修改後確認（edited_value）
    resolved_at         timestamptz,   -- confirm / dismiss 時間
    created_at          timestamptz NOT NULL DEFAULT now()
);
```
...
| id | uuid | PK | — |
| tenant_id | uuid | NOT NULL, FK | RLS |
| contact_id | uuid | NOT NULL, FK, CASCADE | 建議歸屬的客戶 |
| distributor_id | uuid | NOT NULL, FK | 負責教練 |
| inbound_message_id | uuid | NOT NULL, FK | 來源來訊 |
| field | text | NOT NULL, CHECK | 建議寫入的活檔案欄位 |
| suggested_value | text | NOT NULL | AI 建議內容 |
| evidence | text | nullable | 來訊摘錄佐證（PII，加密存） |
| status | text | NOT NULL DEFAULT 'pending', CHECK | pending / confirmed / dismissed |
| confirmed_value | text | nullable | 教練修改後實際入檔的值 |
| resolved_at | timestamptz | nullable | 確認或忽略時間 |
| created_at | timestamptz | NOT NULL DEFAULT now() | — |

**索引**：
```sql
CREATE INDEX idx_contact_suggestions_contact ON care.contact_suggestions(contact_id, status);
CREATE INDEX idx_contact_suggestions_pending ON care.contact_suggestions(distributor_id, status)
    WHERE status = 'pending';
```

**RLS Policy**：同 `contacts`（distributor 看自己、leader 看全租戶）
...
| voice_clips（storage，未發送） | 7 天後清除 | APScheduler 依 expires_at 刪除（GCS Lifecycle 作 fallback） |
| voice_clips（storage，已經 OA 發送） | sent_at + 30 天後清除 | APScheduler 依 retention_until 刪除（客戶重播期內保留） |
| inbound_messages.text（來訊原文） | 跟隨客戶資料刪除請求 | application-level 加密存；DELETE 請求級聯清除 |
| contact_suggestions.evidence | 跟隨客戶資料刪除請求 | PII 加密存；級聯清除 |
...
30. contact_suggestions（依賴 tenants + contacts + distributors + inbound_messages）
```
